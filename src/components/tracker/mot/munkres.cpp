...
Munkres::Munkres(Eigen::MatrixXf *matrix) {
...
int Munkres::stage_3() {
  ...
  for (int i = 0; i < node_counter; i++) {
    ...
    c = node_sequence[i][1];
#if DEBUG
    std::cout << "(" << i << ") [" << std::setw(2) << r << "," << std::setw(2)
              << c << "]\n";
#endif /* DEBUG */
    if (i % 2 == 0) {
...
bool Munkres::find_uncovered_zero(int &r, int &c) {
  for (int i = 0; i < m_size; i++) {
    if (m_covered_R[i]) {
      continue;
    }
...
float Munkres::find_uncovered_min() {
...
bool Munkres::find_star_by_row(const int &row, int &c) {
  for (int j = 0; j < m_size; j++) {
...
bool Munkres::find_prime_by_row(const int &row, int &c) {
  for (int j = 0; j < m_size; j++) {
...
bool Munkres::find_star_by_col(const int &col, int &r) {
  for (int i = 0; i < m_size; i++) {
...
void Munkres::substract_min_by_row() {
  for (int i = 0; i < m_size; i++) {
...
void Munkres::substract_min_by_col() {
  for (int j = 0; j < m_size; j++) {
...
void Munkres::show_state_matrix() {
  std::cout << "==============================================\n";
  ...
  for (int i = 0; i < m_rows; i++) {
    if (m_match_result[i] != -1) {
      ...
      std::cout << "(" << std::setw(2) << i << "," << std::setw(2) << j
                ...
                << (*m_original_matrix)(i, j) << std::endl;
      total_cost += (*m_original_matrix)(i, j);
    }
  }

  std::cout << "total cost = " << std::setw(10) << std::setprecision(4)
            << total_cost << std::endl;
}